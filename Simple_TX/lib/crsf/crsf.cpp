...
void CRSF::crsfPrepareDataPacket(uint8_t packet[], int16_t channels[]) {
    ...
    */
    ...
    packet[6] = (uint8_t)((channels[2] & 0x07FF) >> 2);                                 /* throttle! */
    packet[7] = (uint8_t)((channels[2] & 0x07FF) >> 10 | (channels[3] & 0x07FF) << 1);  
    packet[8] = (uint8_t)((channels[3] & 0x07FF) >> 7 | (channels[4] & 0x07FF) << 4);   /* rudder! */
    packet[9] = (uint8_t)((channels[4] & 0x07FF) >> 4 | (channels[5] & 0x07FF) << 7);   /* switch! */
